...
class Parser
{
    ...
    private ProgrNode ? Parsing()
    {
        ProgrNode ? programNode = new();
    ...
    private Expr CheckBodyStat()
    {
       switch(Peek().TokenType)
       {
            ...
            default:
                throw new Exception("Invalid Token in the currrenyt context");
       }
    }
    ...
    private Expr ParsingAsigAndDoc()
    {
        ...
        if(Match(TokenType.EQUAL) || Match(TokenType.PLUS_EQUAL) || Match(TokenType.MINUS_EQUAL)){
            ...
            expr = new AssigmentExpr(new ID(previous.Value!),Previous().Value!,Expretion());
        }else{
        ...
        //Aqui mismo se puede hacer el casting de las DocExpretions
        return expr;
    }
    ...
    private Expr NegMinUnary() { // Falta acomodar esto para el ++ y el -- se me ocurre hacer heredar a cada una por separado 
